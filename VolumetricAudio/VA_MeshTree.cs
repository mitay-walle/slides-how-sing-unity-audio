		...
		private void Pack(Node node, List<VA_Triangle> tris)
		{
			...
			else
			{
					...
					case 0:
						{
							...
							{
								if (triangle.MidX >= pivot) positiveTris.Add(triangle);
								else negativeTris.Add(triangle);
							}
						}
						break;
					...
					case 1:
						{
							foreach (var triangle in tris)
							{
								if (triangle.MidY >= pivot) positiveTris.Add(triangle);
								else negativeTris.Add(triangle);
							}
						}
						break;
					...
					case 2:
						{
							foreach (var triangle in tris)
							{
								if (triangle.MidZ >= pivot) positiveTris.Add(triangle);
								else negativeTris.Add(triangle);
							}
						}
						break;
				...
				if (positiveTris.Count == 0 || negativeTris.Count == 0)
				{
					...
					int split = MathUtility.SafeDivision(tris.Count, 2);
					...
					for (int i = split; i < tris.Count; i++)
					{
				...
				var positiveNode = new Node();
				Nodes.Add(positiveNode);
				Pack(positiveNode, positiveTris);
				node.NegativeIndex = Nodes.Count;
				var negativeNode = new Node();
				Nodes.Add(negativeNode);
				Pack(negativeNode, negativeTris);
			}
		}
